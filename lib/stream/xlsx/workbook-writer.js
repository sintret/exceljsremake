...
function createZipArchiver(zipOptions) {
  if (typeof Archiver === 'function') {
    return Archiver('zip', zipOptions);
  }

  if (Archiver && typeof Archiver.ZipArchive === 'function') {
    return new Archiver.ZipArchive(zipOptions);
  }
...
class WorkbookWriter {
  constructor(options) {
    ...
    this.zip = createZipArchiver(this.zipOptions);
    if (options.stream) {
  ...
  _openStream(path) {
    // archiver@7+ uses archiver-utils + is-stream to validate input streams.
    // StreamBuf (readable-stream) isn't detected there, but WorksheetWriter
    // writes StringBuf chunks, which a normal PassThrough can't accept.
    // So we write into StreamBuf and pipe its Buffer output into PassThrough.
    const stream = new StreamBuf({bufSize: 65536, batch: true});
    const output = new nodeStream.PassThrough();
    stream.pipe(output);
    this.zip.append(output, {name: path});
    stream.on('finish', () => {
      stream.emit('zipped');
      output.end();
    });
