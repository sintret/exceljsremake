...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    // store as buffers to avoid unzipper temp-file edge cases on some platforms
    ...
    for await (const entry of iterateStream(zip)) {
      ...
      switch (entryPath) {
        ...
        case 'xl/sharedStrings.xml':
          this._seenSharedStringsEntry = true;
        ...
        default:
          if (entryPath.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              waitingWorkSheets.push({sheetNo, data: Buffer.concat(chunks)});
              shouldDrain = false;
            }
    ...
    for (const {sheetNo, data} of waitingWorkSheets) {
      const fileStream = nodeStream.Readable.from([data]);
