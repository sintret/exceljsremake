  ...
  prepare(model) {
    ...
    model.sheets.forEach(sheet => {
      if (sheet.pageSetup && sheet.pageSetup.printArea) {
        const sheetName = escapeSheetName(sheet.name);
        sheet.pageSetup.printArea.split('&&').forEach(printArea => {
          const decoded = colCache.decode(printArea);
          const tl = colCache.decodeAddress(decoded.tl);
          const br = colCache.decodeAddress(decoded.br);
          const definedName = {
            name: '_xlnm.Print_Area',
            ranges: [`'${sheetName}'!${tl.$col$row}:${br.$col$row}`],
            localSheetId: index,
          };
      ...
      if (sheet.pageSetup && (sheet.pageSetup.printTitlesRow || sheet.pageSetup.printTitlesColumn)) {
        ...
        if (sheet.pageSetup.printTitlesColumn) {
          ...
          ranges.push(`'${sheetName}'!$${titlesColumns[0]}:$${titlesColumns[1]}`);
        }

        if (sheet.pageSetup.printTitlesRow) {
          const titlesRows = sheet.pageSetup.printTitlesRow.split(':');
          ranges.push(`'${sheetName}'!$${titlesRows[0]}:$${titlesRows[1]}`);
        }
